...
- (IBAction)pushSet:(id)sender{
    NSLog(@"K810FnKey: start setting");

    if ([button.title isEqual: @"end"]) {
        NSLog(@"K810FnKey: exit");
        exit(0);
    }
    ...
    if (CFGetTypeID(hidManager) != IOHIDManagerGetTypeID()) {
        NSLog(@"K810FnKey: fail to create IOHIDManager");
        return;
    }
    ...
    long HID_PRODUCT_ID_K810 = 0xB319;
    ...
    CFDictionarySetValue(matchDict,
                         ...
                         CFNumberCreate( kCFAllocatorDefault, kCFNumberSInt32Type, &HID_PRODUCT_ID_K810 ));
...
static void Handle_DeviceRemovalCallback(void *inContext,
    ...
    NSLog(@"\nK810 device removed: %p", (void *)inIOHIDDeviceRef);
